...
template <typename T, size_t sz> bool boo<T,sz>::bear(){;};
template <typename T, size_t sz> boo<T,sz>::boo(){
    _rdPtr=0;
    _wrPtr=0;
    _maxSize=0;
};
template <typename T, size_t sz> IRAM_ATTR bool boo<T,sz>::push(T* data){
//template <typename T, size_t sz> IRAM_ATTR  bool  ringBuffer<T, sz>::push(T* data){
    bool verbose = true;
...
template <typename T, size_t sz> bool boo<T,sz>::pop(t& data){
...
template <typename T, size_t sz> bool boo<T, sz>::isEmpty() { return(_maxSize == 0); }
//template <typename T, size_t sz> bool boo<T,sz>::isEmpty(){;};
template <typename T, size_t sz> bool boo<T,sz>::isFull() { return(_maxSize == sz); }
//template <typename T, size_t sz> bool boo<T,sz>::isFull(){;};
//
*/
//*------------------------------------------------------
//* Simple Ring Buffer class for interrupt driven inputs
template <typename T, size_t sz>
class ringBuffer {
    static_assert(sz > 0, "RingBuf with size 0 are forbidden");
    static_assert(sz <= __UINT8_MAX__, "RingBuf with size greater than 2^8 (0xff) are forbidden");
